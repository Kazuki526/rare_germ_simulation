...
void Population::correlation_ns(){
  ...
  std::vector<std::size_t> non_rare_num(N,0), syn_rare_num(N,0), cont_rare_num(N,0);
  std::vector<std::size_t> non_notrare_num(N,0), syn_notrare_num(N,0), cont_notrare_num(N,0);
  ...
  for(std::size_t i=0; i < N;i++){
    ...
    for(std::size_t cn_het: individuals[i].get_cont_non_het()){
      if(num_cont_non_mutation[cn_het] < rare){cont_rare_num[i]++;}else{cont_notrare_num[i]++;}
    }
    for(std::size_t cn_hom: individuals[i].get_cont_non_hom()){
      if(num_cont_non_mutation[cn_hom] < rare){cont_rare_num[i]+=2;}else{cont_notrare_num[i]+=2;}
    }
    mutater_num.push_back(individuals[i].get_mutater());
    mutation_rate.push_back(individuals[i].get_mut_r());
  }
  double nonav,synav,contav;
  ...
  contav = (double)std::accumulate(cont_rare_num.begin(),cont_rare_num.end(),0.0)/N;
  ...
  double nonv=0.0, synv=0.0,contv=0.0, nonsynv=0.0,nonconv=0.0, mutv=0.0, mutrv=0.0;
  ...
  mut0_rare_cont_num=0.0; mut1_rare_cont_num=0.0; mut2_rare_cont_num=0.0;
  mut0_notrare_cont_num=0.0; mut1_notrare_cont_num=0.0; mut2_notrare_cont_num=0.0;
  for(std::size_t i=0; i < N; i++){
    nonv+=(double)((non_rare_num[i]-nonav)*(non_rare_num[i]-nonav));
    synv+=(double)((syn_rare_num[i]-synav)*(syn_rare_num[i]-synav));
    contv+=(double)((cont_rare_num[i]-contav)*(cont_rare_num[i]-contav));
    nonsynv+=(double)((non_rare_num[i]-nonav)*(syn_rare_num[i]-synav));
    nonconv+=(double)((non_rare_num[i]-nonav)*(cont_rare_num[i]-contav));
    ...
    if(mutater_num[i]==0){mut0++;
      ...
      mut0_rare_cont_num+=cont_rare_num[i];mut0_notrare_cont_num+=cont_notrare_num[i];
      ...
      mut1_rare_cont_num+=cont_rare_num[i];mut1_notrare_cont_num+=cont_notrare_num[i];
    }else{mut2++;
      ...
      mut2_rare_cont_num+=cont_rare_num[i];mut2_notrare_cont_num+=cont_notrare_num[i];
    }
  }
  if(mut2==0){mut2=1;}
  ...
  mut0_rare_cont_num/=mut0;
  mut1_rare_cont_num/=mut1;
  mut2_rare_cont_num/=mut2;
  mut0_notrare_cont_num/=mut0;
  mut1_notrare_cont_num/=mut1;
  mut2_notrare_cont_num/=mut2;
  rare_tsg_non_sd = std::sqrt((double)nonv/N);
  rare_tsg_syn_sd = std::sqrt((double)synv/N);
  rare_cont_non_sd = std::sqrt((double)contv/N);
  rare_non_syn_correlation = (double)(nonsynv/N)/(rare_tsg_non_sd*rare_tsg_syn_sd);
  rare_non_cont_correlation = (double)(nonconv/N)/(rare_tsg_non_sd*rare_cont_non_sd);
